...
function buildBundle(
  ...
  if (!packagerInstance) {
    ...
    const manifestFileContents = args.manifestFile
      ? JSON.parse(fs.readFileSync(args.manifestFile, 'utf8'))
      : null

    const manifestOutputFile = args.manifestOutput
      ? args.manifestOutput
      : null

    const postProcessModules = args.postProcessModules
      ? require(args.postProcessModules)(manifestFileContents, manifestOutputFile)
      : config.postProcessModules;

    const createModuleIdFactory = args.createModuleIdFactory
      ? require(args.createModuleIdFactory)(manifestFileContents, manifestOutputFile)
      : null;
